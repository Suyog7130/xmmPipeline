...
class xmmObj:
    ...
    def findObsIDs (self):
        ...
        if not os.path.isfile(workdir+"/"+"browse_extract_wget.pl"):
            ...
            #wget.download("https://heasarc.gsfc.nasa.gov/FTP/heasarc/software/web_batch/browse_extract_wget.pl", out=workdir)
        ...
        df.columns = cols
        #offsets = [s.split()[0].strip() for s in df['_Search_Offset'].dropna()]
        #df = df[df['_Search_Offset'].astype(float)>1.0]
        obsIDs = []
